...
/**
 * @name NamecheapAccounts
 * @description NamecheapAccounts Controller
 */
class NamecheapAccounts extends Controller
{
    /**
     * @app
     * @readwrite
    ...
    { 
        ...
        $this->masterView->set([
            'dns_management' => 'true',
            'namecheap_accounts' => 'true',
            'namecheap_accounts_show' => 'true'
        ]);
    ...
    { 
        ...
        if(count($data))
        {
            ...
            die(json_encode(DataTable::init($data,'admin.namecheap_accounts',$columns,new Namecheap(),'namecheap-accounts','ASC')));
        }
    }
    ...
    { 
        ...
        $account = Namecheap::first(Namecheap::FETCH_ARRAY,['id = ?',$id]);
        ...
        else
        {
            ...
            Page::registerMessage('error','Invalid namecheap account id !');
    ...
    { 
        ...
        {        
            ...
            $account->setName($this->app->utils->arrays->get($data,'namecheap-name'));
            $account->setStatus($this->app->utils->arrays->get($data,'namecheap-status'));
            $account->setUsername($this->app->utils->arrays->get($data,'namecheap-user'));
            $account->setApiKey($this->app->utils->arrays->get($data,'namecheap-api-key'));
            $account->setWhiteListedIp($this->app->utils->arrays->get($data,'namecheap-whitelised-ip'));
