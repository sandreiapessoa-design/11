...
/**
 * @name NamecomAccounts
 * @description NamecomAccounts Controller
 */
class NamecomAccounts extends Controller
{
    /**
     * @app
     * @readwrite
    ...
    { 
        ...
        $this->masterView->set([
            'dns_management' => 'true',
            'namecom_accounts' => 'true',
            'namecom_accounts_show' => 'true'
        ]);
    ...
    { 
        ...
        if(count($data))
        {
            ...
            die(json_encode(DataTable::init($data,'admin.namecom_accounts',$columns,new Namecom(),'namecom-accounts','ASC')));
        }
    }
    ...
    { 
        ...
        $account = Namecom::first(Namecom::FETCH_ARRAY,['id = ?',$id]);
        ...
        else
        {
            ...
            Page::registerMessage('error','Invalid namecom account id !');
    ...
    { 
        ...
        {        
            ...
            $account->setName($this->app->utils->arrays->get($data,'namecom-name'));
            $account->setStatus($this->app->utils->arrays->get($data,'namecom-status'));
            $account->setUsername($this->app->utils->arrays->get($data,'namecom-user'));
            $account->setApiKey($this->app->utils->arrays->get($data,'namecom-api-key'));
            $account->setWhiteListedIp($this->app->utils->arrays->get($data,'namecom-whitelised-ip'));
