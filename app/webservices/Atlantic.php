...
# models
use IR\App\Models\Admin\AtlanticAccount as AtlanticAccount;
use IR\App\Models\Admin\AtlanticProcess as AtlanticProcess;
...
/**
 * @name Atlantic
 * @description Atlantic WebService
 */
class Atlantic extends Base
{
    /**
     * @app
     * @readwrite
    ...
    { 
        ...
        $access = Permissions::checkForAuthorization(Authentication::getAuthenticatedUser(),'AtlanticInstances','create');
        ...
        $account = AtlanticAccount::first(AtlanticAccount::FETCH_ARRAY,['id = ?',intval($this->app->utils->arrays->get($parameters,'account-id',0))],['id']);
        ...
        # call iresponse api
        Api::call('Atlantic','createInstances',['process-id' => $process->insert()],true,LOGS_PATH . DS . 'cloud_apis' . DS . 'inst_atln_' . $account['id'] . '.log');
    ...
    { 
        ...
        # call iresponse api
        $result = Api::call('Atlantic','stopProcesses',['processes-ids' => $processesIds]);
    ...
    { 
        ...
        # check for permissions
        $access = Permissions::checkForAuthorization(Authentication::getAuthenticatedUser(),'AtlanticInstances','main');
        ...
        # call iresponse api
        $result = Api::call('Atlantic','executeInstancesActions',['instances-ids' => $instancesIds,'action' => $action]);
