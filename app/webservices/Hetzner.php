...
use IR\App\Models\Admin\HetznerProcess as HetznerProcess;
...
/**
 * @name Hetzner
 * @description Hetzner WebService
 */
class Hetzner extends Base
{
    /**
     * @app
     * @readwrite
    ...
    { 
        ...
        $access = Permissions::checkForAuthorization(Authentication::getAuthenticatedUser(),'HetznerInstances','create');
        ...
        $account = HetznerAccount::first(HetznerAccount::FETCH_ARRAY,['id = ?',intval($this->app->utils->arrays->get($parameters,'account-id',0))],['id']);
        ...
        # call iresponse api
        Api::call('Hetzner','createInstances',['process-id' => $process->insert()],true,LOGS_PATH . DS . 'cloud_apis' . DS . 'inst_htz_' . $account['id'] . '.log');
    ...
    { 
        ...
        # call iresponse api
        $result = Api::call('Hetzner','stopProcesses',['processes-ids' => $processesIds]);
    ...
    { 
        ...
        # check for permissions
        $access = Permissions::checkForAuthorization(Authentication::getAuthenticatedUser(),'HetznerInstances','main');
        ...
        # call iresponse api
        $result = Api::call('Hetzner','executeInstancesActions',['instances-ids' => $instancesIds,'action' => $action]);
