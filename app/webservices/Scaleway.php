...
# models
use IR\App\Models\Admin\ScalewayAccount as ScalewayAccount;
use IR\App\Models\Admin\ScalewayProcess as ScalewayProcess;
...
/**
 * @name Scaleway
 * @description Scaleway WebService
 */
class Scaleway extends Base
{
     /**
     * @app
     * @readwrite
    ...
    { 
        ...
        $access = Permissions::checkForAuthorization(Authentication::getAuthenticatedUser(),'ScalewayInstances','create');
        ...
        $account = ScalewayAccount::first(ScalewayAccount::FETCH_ARRAY,['id = ?',intval($this->app->utils->arrays->get($parameters,'account-id',0))],['id']);
        ...
        # call iresponse api
        Api::call('Scaleway','createInstances',['process-id' => $process->insert()],true,LOGS_PATH . DS . 'cloud_apis' . DS . 'inst_scaw_' . $account['id']);
    ...
    { 
        ...
        # call iresponse api
        $result = Api::call('Scaleway','stopProcesses',['processes-ids' => $processesIds]);
    ...
    { 
        ...
        # check for permissions
        $access = Permissions::checkForAuthorization(Authentication::getAuthenticatedUser(),'ScalewayInstances','main');
        ...
        # call iresponse api
        $result = Api::call('Scaleway','executeInstancesActions',['instances-ids' => $instancesIds,'action' => $action]);
